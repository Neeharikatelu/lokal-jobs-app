...
type Job = {
  ...
  description: string;
  image: string;
  skills?: string[]; // Extracted from contentV3
  benefits?: string[]; // Extracted from job_tags
};
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      const mappedJobs = data.data.map((job: any) => ({
        ...
        image: job.creatives?.[0]?.file || 'https://via.placeholder.com/150', // Use the first creative image or a placeholder
        skills: job.contentV3?.V3?.map((field: any) => field.field_value) || [], // Extract skills or other details
        benefits: job.job_tags?.map((tag: any) => tag.value) || [], // Extract job tags as benefits
      }));
